...
def insert_regions(cursor, regions):
    for region in regions:
        cursor.execute(
            "INSERT INTO region (code, nom) VALUES (%s, %s) ON CONFLICT (code) DO NOTHING",
...
def insert_departements(cursor, departements):
    for departement in departements:
        cursor.execute(
            "INSERT INTO departement (code, nom, \"codeRegion\") VALUES (%s, %s, %s) ON CONFLICT (code) DO NOTHING",
...
def insert_communes(cursor, communes):
    ...
    for commune in communes:
        ...
        cursor.execute("SELECT 1 FROM departement WHERE code = %s", (code_departement,))
        ...
        # Insérer la commune
        cursor.execute(
            "INSERT INTO commune (code, nom, \"codeDepartement\", latitude, longitude) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (code) DO NOTHING",
